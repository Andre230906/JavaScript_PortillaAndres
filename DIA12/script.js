...
async function fetchUnoCards() {
    const response = await fetch("https://deckofcardsapi.com/api/deck/new/draw/?count=108");
    const data = await response.json();
    return data.cards;
}

async function createUnoCards() {
    const unoCards = await fetchUnoCards();
    unoCards.forEach(card => {
        const unoCard = {
            color: convertColor(card.suit),
            symbol: convertSymbol(card.value)
        };
        deck.push(unoCard);
    });
}

function convertColor(suit) {
    switch (suit) {
        case "HEARTS":
            return "red";
        case "DIAMONDS":
...
function renderDeck() {
    ...
    else if (carta.symbol == "color") symbolHTML = '<ion-icon name="color-palette-outline"></ion-icon>';
